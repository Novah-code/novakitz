...
Deno.serve(async (req) => {
  try {
    ...
    const expectedToken = Deno.env.get('MONTHLY_REPORT_SECRET');
    ...
    if (usersError) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to fetch premium users' }), { status: 500 });
    }

    if (!users || users.length === 0) {
      return new Response(JSON.stringify({ message: 'No premium users found', generatedReports: 0 }), { status: 200 });
    }
