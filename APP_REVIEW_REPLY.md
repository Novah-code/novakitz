...
>
> - **빌드 3 선택 + 재제출** — 2026-09-14 완료. `Waiting for Review`, 4개 항목 전부.
>
> **끝났습니다. 남은 것 없습니다.** 결과를 기다리는 단계입니다.
>
> ### 한 번 걸렸던 것 — 다음에도 똑같이 걸립니다
>
> 9/10에 답장을 보낸 뒤 나흘 동안 아무 일도 일어나지 않았습니다. **답장은 재제출이
> 아니기 때문입니다.** 버전은 `Rejected` 상태로 그대로 멈춰 있었고, 구독 3개도 그
> 뒤에서 같이 막혀 있었습니다.
>
> 그리고 그동안 빌드는 `1.0 (2)`였습니다. 업로드는 9/10에 했지만 **버전 페이지에서
> 선택하는 단계**가 빠져 있었어요. 그대로 심사에 들어갔다면 심사자는 Restore
> Purchases도 없고 Terms 링크도 안 열리는 빌드 2를 열어놓고, 둘 다 작동하는 영상을
> 본 직후였을 겁니다.
>
> **다음에 반려를 받으면 순서는 이렇습니다:** 고친다 → 빌드 올린다 → **버전
> 페이지에서 빌드를 고른다** → **다시 제출한다** → 상태가 `Waiting for Review`로
> 바뀐 것을 눈으로 확인한다. 마지막 두 개가 빠지기 쉽습니다.
>
